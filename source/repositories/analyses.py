...
class AnalysisRepository:
    ...
    def get_analysis_status_by_hash(self, document_hash: str) -> ProcurementAnalysisStatus | None:
        """
        Retrieves the status of the most recent analysis for a given document hash.
        """
        sql = text(
            """
            SELECT status
        ...
        with self.engine.connect() as conn:
            result = conn.execute(sql, {"document_hash": document_hash}).scalar_one_or_none()

        if result:
            return ProcurementAnalysisStatus(result)

        return None

    def get_latest_analysis_by_hash(self, document_hash: str) -> AnalysisResult | None:
        """
