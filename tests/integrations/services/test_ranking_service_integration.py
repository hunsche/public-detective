...
def _expected_priority(
    *,
    ...
    adjusted_impact = potential_impact * (1 + config.RANKING_W_VOTES * vote_factor)
    priority = (
        (config.RANKING_W_IMPACT * adjusted_impact)
        + (config.RANKING_W_QUALITY * quality_score)
        - (config.RANKING_W_COST * float(estimated_cost))
    )
    return int(priority)
...
def test_calculate_priority_quality_penalties_applied(ranking_service: RankingService) -> None:
    ...
    assert procurement.priority_score == int(ranking_service.config.RANKING_W_QUALITY * 35)
