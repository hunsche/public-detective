...
@pytest.fixture
def client(db_session: Engine) -> Generator[TestClient, None, None]:
    # db_session fixture ensures DB is ready and env vars are set
    yield TestClient(app)


def test_home_page_structure(client: TestClient) -> None:
    ...
    href = analyses_link.get("href")
    assert isinstance(href, str)
    assert href.endswith("/analyses")


def test_analyses_page_browser_access(client: TestClient) -> None:
...
def test_analyses_page_htmx_request(client: TestClient) -> None:
    ...
    if card is None:
        ...
        # Use lambda to avoid mypy call-overload error with string argument
        assert soup.find("p", string="Nenhuma análise encontrada.") is not None  # type: ignore[call-overload]
    else:
        assert card is not None


def test_search_functionality_htmx(client: TestClient) -> None:
...
def test_analysis_detail_page(client: TestClient) -> None:
    ...
    # However, since we don't have easy access to the DB session here
    # (client fixture doesn't return it directly, though we could request it),
    ...
    if detail_link:
        ...
        response = client.get(str(detail_url))
        assert response.status_code == 200
        soup = BeautifulSoup(response.text, "html.parser")
