...
def test_extract_from_zip_with_dir(repo):
    ...
    assert ("dir/file1.txt", b"content1") in extracted_files
...
@patch("requests.get")
def test_get_updated_procurements_with_raw_data_empty_data(mock_get, repo):
    """Tests handling of API response with an empty data list."""
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"data": [], "totalPaginas": 1}
    mock_get.return_value = mock_response

    procurements = repo.get_updated_procurements_with_raw_data(date(2025, 1, 1))
...
def test_get_updated_procurements_with_raw_data_validation_error(mock_get, repo):
    """Tests handling of a validation error when processing API response."""
    mock_response = MagicMock()
    mock_response.status_code = 200
    # This is invalid because 'data' should be a list of dicts, not a dict
