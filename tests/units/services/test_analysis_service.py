...
def test_run_ranked_analysis_manual_budget(mock_dependencies: dict) -> None:
    ...
    with (
        patch.object(service, "_calculate_estimated_cost", return_value=Decimal("1.00")),
        patch.object(service, "run_specific_analysis") as mock_run_specific,
    ):
        service.run_ranked_analysis(
            use_auto_budget=False, budget=Decimal("1.50"), budget_period=None, zero_vote_budget_percent=10
        )
...
def test_run_ranked_analysis_budget_exhausted(mock_dependencies: dict) -> None:
    ...
    with (
        patch.object(service, "_calculate_estimated_cost", return_value=Decimal("2.00")),
        patch.object(service, "run_specific_analysis") as mock_run_specific,
    ):
        service.run_ranked_analysis(
            use_auto_budget=False, budget=Decimal("1.50"), budget_period=None, zero_vote_budget_percent=10
